...
static int16_t Vq_subvec_s(float * lsf_r1, float * lsf_r2,
			  ...
			  int16_t dico_size)
{
	float dist_min, dist1, dist2, temp1, temp2;
	const float *p_dico;
	int32_t i, index = 0;
	int16_t sign = 0;
			   ...
			   float gain_cand[], int32_t gain_cind[])
{
	...
	err_min = (float) fabsf(*gain - qua_gain_pitch[0]);
	index = 0;

	for (i = 1; i < NB_QUA_PITCH; i++) {
		if (qua_gain_pitch[i] <= gp_limit) {
			err = (float) fabsf(*gain - qua_gain_pitch[i]);
			 ...
			 pos_max[], int16_t nb_track, int32_t ipos[], int16_t step)
{
	...
	k_cn = (float) (1 / sqrtf(sum));
	sum = 0.01F;
	sum += (float) Dotproduct40(dn, dn);
	k_dn = (float) (1 / sqrtf(sum));
			  ...
			  int32_t * qua_ener_index)
{
	...
	err_min = fabsf(*gain - (gcode0 * *p++));
	index = 0;

	for (i = 1; i < NB_QUA_CODE; i++) {
		err = fabsf(*gain - (gcode0 * *p++));
				 ...
				 float gain_pit, float en[], float * ltpg)
{
	...
	if (en[3] > 0 && en[0] != 0) {
		/* gain = ResEn / LTPResEn */
		pred_gain = en[0] / en[3];
		*ltpg = (float) (log10f(pred_gain) / 0.301029999566398114F);
	} else {
		*ltpg = 0;
	}
}
		       ...
		       float * alpha)
{
	...
	if (ltpg <= 0.3321928F /*LTP_GAIN_THR1 */ ) {
		...
		if (ltpg <= 0.6643856F /*LTP_GAIN_THR2 */ ) {
			adapt = 1;
		} else {
			adapt = 2;
		}
	...
	if (adapt == 0) {
		if (filt > 0.66443F) {
			...
			if (filt < 0) {
				...
				result = (float) (0.5F - 0.75257499F * filt);
			}
		}
	} else {
		result = 0;
					...
					int32_t * qua_ener_index)
{
	...
	coeff[0] = (float) (sqrtf(alpha * en[0]));
		       ...
		       float speech[], float * log_en_hist)
{
	...
	*hist_ptr += 1;
	...
	if (frame_en > 1) {
		log_en_hist[*hist_ptr] = (float) (log10f(frame_en * 0.00625F) *
						    1.660964F);
	} else {
...
int vad2(vadState * st, float * farray_ptr)
{
  ...
  /****** Executable code starts here ******/
	...
	peak2avg = (avg < 1. / NORM_ENRG) ? 0. : 10. * log10f(peak / avg);
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++) {
		snr = 10.0 * log10f((float)st->ch_enrg[i] / st->ch_noise[i]);
		if (snr < 0.0)
			snr = 0.0;
...
#else
		tce_db =
		    (96.0F - 22.0F - 18.061799739838869F -
		     10.0F * log10f(NORM_ENRG));
#endif
	...
	for (i = LO_CHAN; i <= HI_CHAN; i++)
		ch_enrg_db[i] = 10.0F * log10f(st->ch_enrg[i]);
	...
	if (st->Lframe_cnt == 1)
		...
		for (i = LO_CHAN; i <= HI_CHAN; i++)
			ch_enrg_dev +=
			    fabsf(st->ch_enrg_long_db[i] - ch_enrg_db[i]);
...
static float phs_tbl[SIZE];	/* holds the complex sinusoids */
...
void fill_tbl()
{
	...
	delta_f = -PI / (float)SIZE_BY_TWO;
	for (i = 0; i < SIZE_BY_TWO; i++) {
		theta = delta_f * (float)i;
		phs_tbl[2 * i] = cosf(theta);
		phs_tbl[2 * i + 1] = sinf(theta);
	}
	return;
}				/* end fill_tbl () */
